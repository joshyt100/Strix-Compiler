(module
  ...
  (data (i32.const 0) "---------\00black\00White\00#600000\00#000060\00white\00#C0C0C0\00#006000\00Winner: \00Tie game!\00yellow\00Test: Canvas clicks with tic-tac-toe\00MouseClick\00MouseMove\00")
  ...
  (func $Init
    ...
    (global.set $var1)
    ;; Calculate RHS for assignment.
    (i32.const 88)  ;; Char literal 'X' → ASCII 88
    (global.set $var2)
    ;; Calculate RHS for assignment.
  ...
  (func $Fun1 (result i32)
    (local $var5 i32) ;; Declare var 'row'
    (local $var6 i32) ;; Declare var 'col'
    (local $var7 i32) ;; Declare var 'id'
    (local $var8 i32) ;; Declare var 'win_char'
    (local $var9 i32) ;; Declare var 'win_label'
    (i32.const 10)  ;; Literal string "black"
    (call $setStrokeColor)  ;; Call function LineColor
    (i32.const 16)  ;; Literal string "White"
    ...
    ;; Calculate RHS for assignment.
    (i32.const 0)  ;; Literal value
    (local.set $var5)  ;; Set variable 'row'
    (block $exit1 ;; Outer block for breaking while loop.
      (loop $loop1 ;; Inner loop for continuing while.
        ...
        ;; Calculate RHS for assignment.
        (i32.const 0)  ;; Literal value
        (local.set $var6)  ;; Set variable 'col'
        (block $exit2 ;; Outer block for breaking while loop.
          (loop $loop2 ;; Inner loop for continuing while.
            ...
            (local.get $var6)  ;; Variable 'col'
            (i32.add)
            (local.set $var7)  ;; Set variable 'id'
            ;; == If Condition ==
            (global.get $var0)
            (local.get $var7)  ;; Variable 'id'
            (i32.add)      ;; Sum pointer and index to get index memory position.
            (i32.load8_u)  ;; Extract value at index.
            (i32.const 88)  ;; Char literal 'X' → ASCII 88
            (i32.eq)
            (if ;; Execute code based on result of condition.
              (then ;; 'then' block
                (i32.const 22)  ;; Literal string "#600000"
                (call $setStrokeColor)  ;; Call function LineColor
              ...
              (else ;; 'else' block
                ...
                (i32.add)      ;; Sum pointer and index to get index memory position.
                (i32.load8_u)  ;; Extract value at index.
                (i32.const 79)  ;; Char literal 'O' → ASCII 79
                (i32.eq)
                (if ;; Execute code based on result of condition.
                  (then ;; 'then' block
                    (i32.const 30)  ;; Literal string "#000060"
                    (call $setStrokeColor)  ;; Call function LineColor
                    (i32.const 12)  ;; Literal value
                    (call $setLineWidth)  ;; Call function LineWidth
                    (i32.const 38)  ;; Literal string "white"
                    (call $setFillColor)  ;; Call function FillColor
                    (local.get $var6)  ;; Variable 'col'
                    (i32.const 200)  ;; Literal value
                    (i32.mul)
                  ...
                  (else ;; 'else' block
                    ...
                    (if ;; Execute code based on result of condition.
                      (then ;; 'then' block
                        (i32.const 44)  ;; Literal string "#C0C0C0"
                        (call $setStrokeColor)  ;; Call function LineColor
                        (i32.const 12)  ;; Literal value
                        (call $setLineWidth)  ;; Call function LineWidth
                        ;; == If Condition ==
    ...
    (if ;; Execute code based on result of condition.
      (then ;; 'then' block
        (i32.const 52)  ;; Literal string "#006000"
        (call $setStrokeColor)  ;; Call function LineColor
        (i32.const 8)  ;; Literal value
        (call $setLineWidth)  ;; Call function LineWidth
        ;; Calculate RHS for assignment.
        (i32.const 45)  ;; Char literal '-' → ASCII 45
        (local.set $var8)  ;; Set variable 'win_char'
        ;; == If Condition ==
        (global.get $var3)
        (i32.const 1)  ;; Literal value
        (i32.eq)
        ...
        ;; Calculate RHS for assignment.
        (i32.const 60)  ;; Literal string "Winner: "
        (local.get $var8)  ;; Variable 'win_char'
        (call $_char2str)
        (call $_str_concat)
        (call $_str_copy)
        (local.set $var9)  ;; Set variable 'win_label'
        ;; == If Condition ==
        (global.get $var3)
        (i32.const 9)  ;; Literal value
        (i32.eq)
        (if ;; Execute code based on result of condition.
          (then ;; 'then' block
            ;; Calculate RHS for assignment.
            (i32.const 69)  ;; Literal string "Tie game!"
            (call $_str_copy)
            (local.set $var9)  ;; Set variable 'win_label'
          ) ;; End 'then'
        ) ;; End 'if'
  ...
  (func $Fun2 (param $var10 i32) (param $var11 i32) (result i32)
    (local $var12 i32) ;; Declare var 'col'
    (local $var13 i32) ;; Declare var 'row'
    ;; == If Condition ==
    (global.get $var3)
    (if ;; Execute code based on result of condition.
      (then ;; 'then' block
    ...
    ;; Calculate RHS for assignment.
    (local.get $var10)  ;; Variable 'x'
    (i32.const 200)  ;; Literal value
    (i32.div_s)
    (local.set $var12)  ;; Set variable 'col'
    ;; Calculate RHS for assignment.
    (local.get $var11)  ;; Variable 'y'
    (i32.const 200)  ;; Literal value
    (i32.div_s)
    (local.set $var13)  ;; Set variable 'row'
    ;; Calculate RHS for assignment.
    (local.get $var13)  ;; Variable 'row'
    (i32.const 3)  ;; Literal value
    (i32.mul)
      ...
      (then ;; 'then' block
        ...
        (if ;; Execute code based on result of condition.
          (then ;; 'then' block
            ...
            (global.get $var1)
            (i32.add)        ;; Sum pointer and index to get index memory position.
            (call $_swap32)  ;; Put the store args in the correct order.
            (i32.store8)     ;; Store the value at index.
            ;; Calculate RHS for assignment.
            (i32.const 79)  ;; Char literal 'O' → ASCII 79
            (global.set $var2)
          ) ;; End 'then'
        ...
        (i32.add)
        (global.set $var4)
        (call $Fun0)  ;; Call function TestWin
  ...
  (func $Fun3 (param $var14 i32) (param $var15 i32) (result i32)
    (local $var16 i32) ;; Declare var 'col'
    (local $var17 i32) ;; Declare var 'row'
    ;; Calculate RHS for assignment.
    (local.get $var14)  ;; Variable 'x'
    (i32.const 200)  ;; Literal value
    (i32.div_s)
    (local.set $var16)  ;; Set variable 'col'
    ;; Calculate RHS for assignment.
    (local.get $var15)  ;; Variable 'y'
    (i32.const 200)  ;; Literal value
    (i32.div_s)
    (local.set $var17)  ;; Set variable 'row'
    ;; Calculate RHS for assignment.
    (local.get $var17)  ;; Variable 'row'
    (i32.const 3)  ;; Literal value
    (i32.mul)
  ...
  (func $Fun4 (result i32)
    (i32.const 86)  ;; Literal string "Test: Canvas clicks with tic-tac-toe"
    (call $setTitle)  ;; Call function SetTitle
    (call $Fun1)  ;; Call function DrawBoard
    (drop) ;; Result not used.
    (i32.const 123)  ;; Literal string "MouseClick"
    (call $addClickFun)  ;; Call function AddClickFun
    (i32.const 134)  ;; Literal string "MouseMove"
