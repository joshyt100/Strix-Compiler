(module
  ...
  (data (i32.const 0) "red\00blue\00#800000\00#000080\00Test: logical && and ||\00")
  (global $free_mem (mut i32) (i32.const 49))
  ...
  (func $Fun0 (result i32)
    (local $var0 i32) ;; Declare var 'x'
    (local $var1 i32) ;; Declare var 'y'
    ;; Calculate RHS for assignment.
    (i32.const 1)  ;; Literal value
    (local.set $var0)  ;; Set variable 'x'
    ;; Calculate RHS for assignment.
    (i32.const 0)  ;; Literal value
    (local.set $var1)  ;; Set variable 'y'
    ;; == If Condition ==
    ...
    (if ;; Execute code based on result of condition.
      (then ;; 'then' block
        (i32.const 0)  ;; Literal string "red"
        (call $setFillColor)  ;; Call function FillColor
      ) ;; End 'then'
      (else ;; 'else' block
        (i32.const 4)  ;; Literal string "blue"
        (call $setFillColor)  ;; Call function FillColor
      ) ;; End 'else'
    ) ;; End 'if'
    ;; == If Condition ==
    ...
    (if ;; Execute code based on result of condition.
      (then ;; 'then' block
        (i32.const 9)  ;; Literal string "#800000"
        (call $setStrokeColor)  ;; Call function LineColor
      ) ;; End 'then'
      (else ;; 'else' block
        (i32.const 17)  ;; Literal string "#000080"
        (call $setStrokeColor)  ;; Call function LineColor
      ) ;; End 'else'
    ) ;; End 'if'
    (i32.const 25)  ;; Literal string "Test: logical && and ||"
